...
class DBconnection
{
    ...
    function __construct() {
        ...
        $this->databaseName = 'hospitalManagement'; //name of your database
        $this->port = '3306';
        $this->debug = true;
        $this->connect();
    ...
    function addSystemLog($logAr){
        if(isset($logAr["action"]) && isset($logAr["log_details"])){
            $fields = "";
            $values = "";
            if(trim($logAr["action"])!=""){
                $fields .= "action, ";
                $values .= "'".$logAr["action"]."', ";
            }
            if(trim($logAr["log_details"])!=""){
                $fields .= "log_details, ";
                $values .= "'".$logAr["log_details"]."', ";
            }
            if(trim($logAr["emp_id"])!=""){
                $fields .= "emp_id, ";
                $values .= "".$logAr["emp_id"].", ";
            }
            ...
            return $this->execute("INSERT INTO system_log (".$fields.") VALUES(".$values.");");
        }else{
            return false;
        }
    ...
    // amar pathano DBconnection file open kor
    ///notepad e
}
